...
const Training = () => {
  return (
    <section className="py-10 px-4">
      <h2 className="text-lg md:text-3xl font-bold uppercase text-secondaryText mb-10 border-b-4 border-primaryText inline-block">
        Education & Training Experience
      </h2>

      {/* Desktop (timeline) hidden on small screens */}
      <div className="hidden md:block relative w-full my-10">
        ...
        {items.map((item, index) => (
          ...
          >
            <div className="flex items-center gap-3 mb-2 text-white">
              {index === 0 ? (
                <FaGraduationCap className="text-primaryText" />
              ) : (
                <FaBriefcase className="text-primaryText" />
              )}
